...
Das \textbf{Sweble Wikitext Components module} soll das Parsen von MediaWikis Wikitext ermöglichen und darüber hinaus eine Engine bieten, die versucht das Verhalten eines MediaWiki zu emulieren. \citep[Vgl.][]{sweble} das bedeutet  \textbf{Sweble} addressiert das Problem, dass selbst Wikis mit einem \citep{sweblePaper} 
...
\medskip
\lstinputlisting[language=JavaScript,caption={Async Beispiel},label={lst:async}]{Inhalt/Code/async.js}
...
Die Architektur des Servers ist zur einfachen Einbindung zusätzlicher Medientypen und Datenquellen vorgesehen. Hier ist zu unterscheiden, ob eine weitere Quelle lediglich zum Liefern von Suchvorschlägen eingebunden werden soll, zum Liefern von Suchergebnissen oder beidem. Dafür sind intern die Datenbank-Objekte \texttt{Suggestion} und \texttt{ResultItem} essentiell.
...
\paragraph{Einbinden eines weiteren Medientyps}
Das Einbinden eines weiteren Medientyps verhält sich äquivalent zur Einbindung eines Dienstes für Suchvorschläge oder -ergebnisse. Die einzelnen Dienste werden nach Medientyp sortiert abgefragt (siehe Listing \ref{lst:getSuggestions}).

\medskip
\lstinputlisting[float,language=JavaScript,caption={Mappen eines IMDb-Results zu einem Suggestion-Objekt},label={lst:getSuggestions}]{Inhalt/Code/getSuggestions.js}


\subsection{Interner Aufbau und Ablauf von Requests}
Auf Abbildung \ref{fig:requestDiagram} ist der grobe Ablauf von Nutzer-Eingabe bis zur Lieferung der resultierenden Medien-Items zu sehen. Bei der Eingabe eines Suchbegriffes werden zunächst mögliche Suchvorschläge gegeben.\footnote{Der schematische Ablauf der Sammlung der Suchvorschläge ist in Abbildung \ref{fig:suggestionsRequestDiagram} zu sehen und wird in Abschnitt \ref{subsec:suggestions} beschrieben} Anhand der Suchvorschläge kann der Nutzer sich ein Item aussuchen, dessen Zeitraum durchsucht werden soll. Anhand diesen Zeitraumes sendet \arbeitstitel \ Sucherequests an die verbundenen APIs und Quellen und gibt ein Set an Suchergebnissen (\texttt{MusicResults}) und abspielbaren Medieninhalten (\texttt{TrailerResults}) zurück.\footnote{Der schematische Ablauf der Sammlung von Suchergebnissen ist auf Abbildung \ref{fig:searchResultsRequestDiagram} zu sehen und wird in Abschnitt \ref{subsec:results} genauer beschrieben.}
