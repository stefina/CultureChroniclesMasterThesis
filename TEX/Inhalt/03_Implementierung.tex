...
\subsubsection{Darstellung von Filmplakaten in unterschiedlichen Formaten}
...
Im Folgenden werden der Funktionsumfang und die Restriktionen der verwendeten API-Wrapper erläutert. Daraufhin wird erklärt, inwiefern die zusätzlichen Module die Implementierung komplettieren.
...
\medskip
\begin{lstlisting}[caption=Async Beispiel,label={lst:async}]
async.parallel({
...
\paragraph{nodebrainz und coverart}

\paragraph{node-imdb-api}


\medskip
\begin{lstlisting}[caption=Request und Cheerio und Async in Action,label={lst:interaction}]
