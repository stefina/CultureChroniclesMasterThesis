...
\medskip
\lstinputlisting[language=HTML,float,caption={IFrame-Code, der zum Einbetten eines Songs von Spotify erforderlich ist. Die auszutauschende Track-ID ist blau hervorgehoben.},label={lst:spotifyIFrame}]{Inhalt/Code/spotifyIFrame.html}
...
\paragraph{mongoose}
\textbf{mongoose} ist ein Objekt-Modellierungs-Tool für die Verwendung von MongoDB \citep{mongodb} unter Node.js \citep[Vgl.][]{mongoose}. Unter anderem bietet \textbf{mongoose} eine unkomplizierte, Schema-basierte Lösung zur Modellierung von Anwendungsdaten und verfügt über Business-Logik, Funktionen wie das Typisieren, Validieren und vereinfacht das Formulieren von Datenbank-Queries \citep[Vgl.][]{mongoose} \uvm 
...
\medskip
\lstinputlisting[float,language=JavaScript,caption={Node-Rest-Client Beispiel},label={lst:nodeRestClient}]{Inhalt/Code/nodeRestClient.js}
...
\medskip
\lstinputlisting[float,language=JavaScript,caption={Request Beispiel},label={lst:request}]{Inhalt/Code/request.js}
...
\medskip
\lstinputlisting[float,language=JavaScript,caption={Cheerio Beispiel},label={lst:cheerio}]{Inhalt/Code/cheerio.js}
...
\medskip
\lstinputlisting[language=JavaScript,float,caption={Verknüpfung von Request, Cheerio und Async},label={lst:interaction}]{Inhalt/Code/requestCheerioAsync.js}
