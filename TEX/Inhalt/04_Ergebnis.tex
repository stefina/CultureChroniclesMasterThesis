...
In Abbildung \ref{fig:screenHome} ist die Web-Applikation in ihrer Ausgangssituation zu sehen. Der Nutzer kann mit einem Klick auf die Selectbox (\glqq Choose a timeframe!\grqq) einen Suchbegriff eingeben. Die Eingabemöglichkeiten, deren Quelle und der jeweils daraus resultierende Zeitpunkt, der für die Suche relevant ist, sind in der Tabelle \ref{tab:eingabeformen} aufgeführt.
...
\begin{table}
	...
	Musiktitel 			& Musiktitel von \textbf{MusicBrainz}  						& Veröffentlichungsdatum \\  \hline
	Interpret			& Musiktitel des Künstlers, die auf \textbf{MusicBrainz} zu finden sind  		& Veröffentlichungsdatum \\
	\hline
	\end{tabular}
	\caption{Eingabeformen, mögliche Quellen und das jeweils aufgelöste Datum}\label{tab:eingabeformen}
\end{table}

Um unnötige Requests zu vermeiden, wird erst bei Eingabe des dritten Zeichens eine Suche gestartet. In Abbildung \ref{fig:screenSuggestions} ist die Anzeige von Suchvorschlägen zu sehen. Anhand des Icons ist zu erkennen, von welcher Quelle der Suchvorschlag kommt. Soweit vorhanden, wird daneben ein Thumbnail des Filmplakats angezeigt, sollte keines gefunden worden sein, wird ein Platzhalter angezeigt. Außerdem werden Titel der vorgeschlagenen Items, sowie das jeweilige Erscheinungsjahr angezeigt. Das Jahr bezeichnet den Zeitraum, der durchsucht wird, wenn der Nutzer darauf klickt.
...
\paragraph{Plus-Icon} Das Plus-Icon verfügt zu diesem Zeitpunkt über keine Funktion, soll in Zukunft jedoch die Funktion bieten, falls es sich bei dem Item um abspielbaren Inhalt handelt, dieses zur Playlist-Queue hinzuzufügen.
...
Nach dem Durchführen einer Suche, kann über das Menu-Icon, welches sich neben dem Suche-Eingabefeld befindet, das Menu aufgerufen werden. Wie in Abbildung \ref{fig:screenSearchDetail} zu sehen ist, werden unter \glqq Search Details\grqq \ das gesuchte Jahr, der Titel des gesuchten Items, sowie der Medien- und Medien-Subtyp angezeigt. Außerdem wird, soweit vorhanden, ein repräsentatives Bild angezeigt, in diesem Fall das Filmplakat von \glqq Ferris Bueller\grq s Day off\grqq . Über den Link der Quelle, in der Abbildung \glqq RottenTomatoes\grqq , kann auf die Webseite des jeweiligen Items auf der Dienstseite navigiert werden. Die Option \glqq Filter Result\grqq \ dient zu diesem Zeitpunkt lediglich als Platzhalter für die zukünftige Funktion des Filterns des Ergebnisses.


\subsection{Funktionalität des Players}
Derzeit wird der Player lediglich mit Film-Trailern befüllt. Die Funktionalität des Players konnte nicht implementiert werden, da die \textbf{TrailerAddict}-API lediglich das Einbetten ihrer Trailer vorsieht. Dementsprechend ist das Result, welches von dieser API zurückkommt, ein HTML-Snippet, das in einem IFrame in \arbeitstitel \ eingebettet wird. Eine externe Steuerung der Funktionalität innerhalb der IFrames ist technisch nicht möglich, sodass die Player-Funktionalität, wie das Abspielen, Skippen oder Pausieren eines Trailers über die Player-Buttons von \arbeitstitel \ nicht bewerkstelligt werden kann.

\section{\arbeitstitel -API}
Durch das \textbf{Express}-Framework ist die implementierte Funktionalität ebenfalls als RESTful-API verwendbar. Zu einer Einbindung der API werden die Routen \lstinline{/suggestions} und \lstinline{/search} bereitgestellt. In den Abbildungen \ref{fig:suggestionAPI} und \ref{fig:resultItemAPI} sind die Dokumentationen der beiden Funktionen zu sehen.\footnote{Bei den Abbildungen \ref{fig:suggestionAPI} und \ref{fig:resultItemAPI} handelt es sich jeweils um Screenshots der mit dem Apiary Framework \citep{apiary} erstellten \arbeitstitel -Dokumentation, welche unter \url{http://docs.culturechronicles.apiary.io/} zu finden ist.}



\begin{figure}[htb]
	\begin{center}
		\includegraphics[width=1\textwidth]{Suggestion.pdf}
		\caption{API-Entität Suggestion}
...
\begin{figure}[htb]
	\begin{center}
		\includegraphics[width=1\textwidth]{ResultItem.pdf}
		\caption{API-Entität ResultItem}
