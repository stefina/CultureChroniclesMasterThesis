...
Zum lokalen Aufsetzen der Web-Applikation muss Node.js sowie MongoDB installiert sein.\footnote{Die genauen Versionen sind der Tabelle \ref{tab:hardSoftware} zu entnehmen und werden hier nicht erneut aufgeführt.} Über die Konsole(Windows) oder im Terminal(Mac OS X) in den Ordner \glqq CultureChronicles\grqq \ navigieren und \lstinline$npm install$ ausführen um alle benötigten Module zu installieren. Zur Ausführung von \arbeitstitel \ muss MongoDB laufen.\footnote{Auf Mac OS X wird MongoDB über den Befehl \lstinline$mongod$ im Terminal gestartet.} Nun kann die Web-Applikation mit dem Befehl \lstinline$grunt$ gestartet werden und ist über \url{localhost:3000/} zu erreichen.